...
public class UserServiceImpl implements UserService {
    ...
    public Page<User> listInstructorUsers(UUID userId, Pageable pageable) {
        return userRepository.findUserByRolesContainingAndIdNot(UserRoleEnum.ROLE_INSTRUCTOR, userId, pageable);
    }

    @Override
    public Page<User> searchInstructorUsersByName(UUID userId, String name, Pageable pageable) {
        return userRepository.findByRolesContainingAndNameContainingIgnoreCaseAndIdNot(UserRoleEnum.ROLE_INSTRUCTOR, name, userId, pageable);
    }
}
