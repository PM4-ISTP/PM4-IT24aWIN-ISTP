...
DELETE FROM course_enrollments WHERE course_id IN (SELECT id FROM courses WHERE title LIKE 'E2E%');
...
DELETE FROM course_instructors WHERE course_id IN (SELECT id FROM courses WHERE title LIKE 'E2E%');
